...
void Timer2A_Handler(void){
  ...
  if (!TaskStarted2A) {
    ...
    uint32_t diff = OS_TimeDifference(LastTime2A, thisTime);
    if(diff>TIMER2_TAILR_R+1){
      ...
      if (jitter > MaxJitter2A) {
        MaxJitter2A = jitter;
      }
      if (MaxJitter2A > MaxJitter) {
        MaxJitter = MaxJitter2A;
      }
      JitterHistogram2A[jitter]++;
    LastTime2A = thisTime;
  }
