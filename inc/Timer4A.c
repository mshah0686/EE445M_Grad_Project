...
void Timer4A_Handler(void){
  ...
  if (!TaskStarted4A) {
    ...
    uint32_t diff = OS_TimeDifference(LastTime4A, thisTime);
    if(diff>TIMER4_TAILR_R+1){
      ...
      if (jitter > MaxJitter4A) {
        MaxJitter4A = jitter;
      }
      if (MaxJitter4A > MaxJitter) {
        MaxJitter = MaxJitter4A;
      }
      JitterHistogram4A[jitter]++;
    LastTime4A = thisTime;
  }
