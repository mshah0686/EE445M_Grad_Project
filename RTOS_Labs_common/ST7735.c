...
void ST7735_FillScreenInit(uint16_t color); /* Special screen initalization that does not use semaphores */
...
void ST7735_InitR(enum initRFlags option) {
  ...
  ST7735_SetCursor(0,0);
  StTextColor = ST7735_YELLOW;
  ST7735_FillScreenInit(0);                 // set screen to black without semaphore accesses
  OS_InitSemaphore(&LCDFree,1);  // means LCD free
}
...
void ST7735_FillScreenInit(uint16_t color) {
  uint8_t hi = color >> 8, lo = color;
  uint8_t x = 0, y = 0, w = _width, h = _height;
...
//        pt        pointer to a null terminated string to be printedz
...
//        value   signed integer to be printed (if INT32_MIN do not print)
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  int len = 0, i = 0;
  char buf[20];
  sprintf(buf, "%s %d", pt, value);
  ...
  if (d) { /* print to bottom */
    ST7735_DrawString(0, 8+l, buf, ST7735_YELLOW);
  }
  else {   /* print to top */
    ST7735_DrawString(0, l, buf, ST7735_YELLOW);
  }
}
