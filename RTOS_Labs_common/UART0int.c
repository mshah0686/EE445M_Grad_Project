...
void static copyHardwareToSoftware(void){
  ...
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (RxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART0_DR_R;
    RxFifo_Put(letter);
    OS_Signal(&RxDataAvailable);        /* Rx FIFO has new data */
  }
}
...
void static copySoftwareToHardware(void){
  ...
  while(((UART0_FR_R&UART_FR_TXFF) == 0) && (TxFifo_Size() > 0)){
    TxFifo_Get(&letter);
    UART0_DR_R = letter;
    OS_Signal(&TxRoomLeft);             /* Tx FIFO has room */
  }
}
...
char UART_InChar(void){
  char letter;
  OS_Wait(&RxDataAvailable);          /* wait until RX FIFO has data */
  RxFifo_Get(&letter);
  return(letter);
}
...
void UART_OutChar(char data){
  OS_Wait(&TxRoomLeft);                 /* wait until TX FIFO has room */
  TxFifo_Put(data);
