...
void Thread7(void){  // foreground thread
  ...
  Jitter(MaxJitter4A, JitterSize, JitterHistogram4A);  // print jitter information
  Jitter(MaxJitter2A, JitterSize, JitterHistogram2A);  // print jitter of second thread
  UART_OutString("\n\r\n\r");
  OS_Kill();
}
...
/* Test case 1: Higher thread semaphore blocking */
/* Lower priority thread, captures semaphore, adds higher thred, higher thread asks for semaphore */

Sema4Type semaphore;            // test of this counting semaphore
uint8_t flag;
void higher_thread() {
	OS_Wait(&semaphore);			// should remove from running list and run lower priority thread 
	if(flag != 1) {
...
void lower_thread() {
	OS_InitSemaphore(&semaphore, 1);
	OS_Wait(&semaphore);
	OS_AddThread(&higher_thread, 128, 0);
	for(int i = 0; i < 1000; i++) {
		PD1 ^= 0x02;
		OS_Sleep(100);
	}
	flag = 1;
	OS_Signal(&semaphore); // should switch to running the higher thread until OS_Kill/
	if(flag == 1) {
...
int TestFallingPriority() {
  OS_Init();           // initialize, disable interrupts
	flag = 0;
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&lower_thread,128, 4); 
	NumCreated += OS_AddThread(&idle_thread, 128, 5);
  OS_Launch(TIME_2MS); // doesn't return, interrupts enabled in here
  return 0;            // this never executes
}
